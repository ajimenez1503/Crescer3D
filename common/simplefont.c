...
// One problem: It does not restore the current shader!
// So you need to set your shader after calling it.
...
"void main(void)"
"{"
"	outColor = texture(tex_text, texCoord);"
"}";
...
//	loadShaders("minimal.vert", "minimal.frag");
	...
	glUniform1i(glGetUniformLocation(sfProgram, "tex_text"), 0); // Texture unit 0	
}



void charToTexture(char *indata, int theChar, char *data)
{
	int row = theChar / 16;
	int col = theChar & 15;
...
void sfDrawString(int h, int v, const char *s)
{
	...
	for (;*s != 0;s++)
	{
		drawChar(h, v-5, *s); // -10 to get to the proper height
		h += 10;
	}
	glBindTexture(GL_TEXTURE_2D, tex);
	glUseProgram(saveprogram);
